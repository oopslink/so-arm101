...
subtitle: 2026-09-17 多轮真机实验：实现链路、多轮执行、具身能力分析与训练示教对照
section_label: 实验报告
...
本节评估的是**GPT-6 Astra在本次工具、相机、机械臂和现场监督条件下表现出的能力**。我们观察到了一条从语言、图像到真实动作的可用链路，也观察到了反复定位错误、接触误判和恢复失败。两类证据必须同时纳入评价。

### 7.1 “直接驱动”证明了哪一层能力

本次Astra选择动作目标，编写或调用控制脚本，并根据执行结果继续决策。关节插值、周期调度、单位换算、总线通信和电机位置闭环由已有软件与硬件完成。因此，能够驱动实物，证明的是**通用模型通过工具参与具身任务的能力**，不是模型自身已经具备精确运动学、实时控制或接触控制能力。

为了避免混淆，可以把当天的表现分为三个层次：

| 层次 | 本次表现 | 可以支持的判断 |
|---|---|---|
| 任务与工具层 | 理解拿杆、敲击、放回、回零；调用相机、脚本和反馈接口；根据纠正改动作 | 已展示从任务描述到硬件操作的组织能力 |
| 视觉与动作对应层 | 能寻找物体、尝试对准和修正，并曾成功抓起和搬运 | 已展示局部可用性，但空间映射和动作结果预测不稳定 |
| 连续物理交互层 | 出现空夹、滑脱、空敲、放回失败及长停顿 | 尚未展示可靠的连续感知、精确接触和完整自主执行能力 |

不能因为第一层可用就推断第三层已经解决；也不能因为放置失败就否认前两层的实际进展。当天更接近**具备视觉理解和工具使用能力、由人监督的机器人任务执行者**，尚不足以称为成熟的自主操作技能。

### 7.2 各项能力的现场证据

以下采用“观察到的表现—失败或限制—判断”的方式，不给缺乏统一量表支持的百分制评分。

| 能力维度 | 正面证据 | 反面证据或限制 | 本次判断 |
|---|---|---|---|
| 语言理解与目标保持 | 能跟随完整任务，并接受“手柄中部”“银色盘面空白区”“先回零再开始”等要求 | 对“右侧”“扭转”仍需现场澄清，动作未总能落实语义 | 能理解并保持任务要求，空间指代需要坐标和观测支持 |
| 视觉语义理解 | 能从双相机图像辨认木鱼、敲杆、夹爪及相对关系 | 图像相近引发新鲜度疑问，局部遮挡和视角变化影响判断 | 足以支持目标寻找与粗定位，不能等同于精确位姿感知 |
| 三维空间与坐标推理 | 能依据画面选择接近、抬升和旋转的尝试方向 | 夹点靠前/靠后、下降不足、扭转方向错误、多关节变化抵消 | 是本次最明显的能力缺口之一；没有可靠的像素到末端运动映射 |
| 任务分解与规划 | 实际推进了寻找、抓取、试抬、搬运、敲击、放置、恢复和回零 | 主要逐段决策，缺少完整路径和阶段验收，局部探索耗时长 | 能组织步骤；尚未证明高效、稳定的多步骤物理规划 |
| 动作结果预测 | 部分关节调整带来了正确的接近、持杆和搬运 | 放置前未正确判断整杆支撑，重新拾取时把杆推动 | 对工具和物体的几何、接触后果预测不稳定 |
| 反馈纠错 | 能接受“没敲到”“不够低”“扭错边”等纠正，再次调整 | 关键纠正多次由用户提出；重复尝试没有保证误差收敛 | 展示了交互式纠错，尚未展示充分的自主错误识别与恢复 |
| 工具使用与工程调整 | 能调用LeRobot，修改调度方式，增加日志并开展小/大幅测试 | 工程修正并未直接带来完整任务成功 | 工具编排是本次较明确的优势，但不替代运动能力 |
| 经验利用 | 后续采用中段夹取、腕部敲击、远近不同步长和独立截图命名 | 同一天仍出现重复方向和位置错误；没有跨会话复测 | 可观察到上下文内适应，不能称为已经学成持久技能 |
| 实时性与连续操作 | 能委托脚本在本地维持约50 Hz下发 | Astra不在20 ms环内处理图像，段间有长等待 | 实时性主要来自控制程序，不能归因于模型推理频率 |
| 完成判断与不确定性处理 | 有试抬、复查和中止后的重新检查，最终可按要求停止并回零 | 三次腕部循环不等于三次有效敲击；松爪不等于放好 | 需要显式物理验收，不能用模型口头判断或程序成功退出代替 |

### 7.3 能理解“怎么做”，为什么仍然做不准

当天用户给出的技巧并不复杂：夹中部、远处大步、近处小步、腕部敲、扭转调整、放在盘面。Astra能够复述并尝试执行这些要求，但语义到动作之间还缺少三个转换。

首先是**从指代到坐标**：“往前一些”“木鱼右侧”必须落到具体参考系中的位移。腕部相机随手腕旋转，二维画面中的方向不能直接复用为基座方向。

其次是**从期望位移到关节动作**：末端往前、往下或转向，需要肩、肘、腕协调变化。对几路关节同时增大角度，可能相互抵消，也可能改变姿态而没有产生所需平移。当天“幅度大一点”之后仍没有稳定靠近目标，正暴露了这一差距。

最后是**从动作执行到物理结果**：闭合是否夹到了杆、提起是否滑脱、槌头是否接触、松爪后是否由盘面支撑，都需要额外判断。模型选择了正确动作类别，并不保证物理条件已经满足。

因此，本次证据更支持“模型能够理解操作意图并组织尝试”，不足以支持“模型已经形成准确、可泛化的身体与环境预测能力”。若要证明后者，应检查它是否能在执行前预测末端移动方向与幅度，并在不同姿态下重复命中，而不只是事后解释画面。

### 7.4 哪些成绩属于模型，哪些由系统与人补足

| 贡献来源 | 当天承担的工作 | 评价时应避免的误归因 |
|---|---|---|
| GPT-6 Astra | 理解任务、看图判断、选择关节目标、组织工具调用、解释反馈、调整脚本与尝试 | 不把LeRobot已有控制能力全部计作模型学会了控制 |
| 控制脚本与LeRobot | 校准读取、关节单位换算、轨迹插值、定时下发、反馈读取、阈值中止 | 50 Hz、限幅和温度保护是程序机制，不是模型每帧推理的成果 |
| 舵机与机构 | 位置闭环、保持和实际运动 | 指令平滑不能保证实际末端同样平滑；硬件跟踪偏差不全是模型推理错误 |
| 现场用户 | 指定抓点、纠正远近与方向、指出空敲、澄清放置区域、决定重启或回零 | 有人协助完成的片段不能作为无人干预自主成功率 |
| 已有工程与实验资料 | 接口、校准、历史代码及后续训练数据对照 | 未运行任务策略不等于从零开始、没有工具或先验支持 |

本次没有系统统计每轮人工介入次数，也没有把通信、传输、模型推理和用户等待分别计时。因此不能把所有段间空档归因于大模型本身，也不能给出严格的自主程度比例。

### 7.5 与训练策略、示教数据应怎样比较

今天没有同场景运行π0，所以不能得出“Astra优于/弱于π0”的实测结论。当前能比较的是Astra驱动产生的行为，与训练集中人类示教的实际运动。

示教提供了连续协调的动作参考；Astra在现场展示了接收自然语言纠正、修改任务流程和调用诊断工具的能力。这些属于不同评价维度。前者的平滑轨迹不能证明训练出的策略在所有场景都可靠，后者的解释和纠错能力也不能证明真实操作已经准确。

对本次系统而言，合理方向是：让Astra处理任务目标、场景变化和恢复选择，让经过验证的本地程序负责空间转换、连续轨迹及接触检查。这样仍然可以不运行专门训练的动作策略；如果未来引入学习到的技能执行器，则应明确标注贡献边界，不能继续把全部表现称为“纯大模型直接控制”。

### 7.6 如何进一步验证大模型的具身能力

下一轮应把“模型会调用工具”和“模型能可靠操作实物”分别测试。以下是实验设计建议，尚未执行。

| 验证问题 | 建议实验 | 核心指标 |
|---|---|---|
| 是否能把视觉误差转成正确动作 | 在有标定真值的不同姿态下，先记录模型预测的方向/位移，再执行并测量 | 方向正确率、末端位移误差、误差是否随修正减少 |
| 是否需要人不断纠正 | 固定场景与接口，预先约定允许的人工介入方式，记录每次提示 | 无纠正完整成功率、每任务介入次数、阶段成功率 |
| 是否能识别失败并恢复 | 在受控条件下设置抓取偏移、杆位置变化或一次未接触 | 自主发现率、误报率、恢复成功率、额外动作数 |
| 是否具备场景适应能力 | 改变杆的位置/朝向及木鱼位置，并保留独立测试布局 | 未见布局成功率、重新定位时间、碰撞/推动物体事件 |
| 改善来自模型还是执行器 | 保持模型与任务一致，对比关节目标接口和标定末端接口；另对比相同路径的分段/连续执行 | 空间误差与任务成功率的变化、连续性、跟踪误差 |
| 是否沉淀了可复用经验 | 将当日技巧整理成固定操作规则，在新会话、相同条件复测 | 首次成功率、重复错误频次、对规则的依赖 |
| 是否能正确判断完成 | 用独立视频标注或接触证据判定抓取、敲击和放置，再与模型判断对照 | 错报成功率、漏报成功率、有效敲击计数误差 |

可先对每个固定条件做至少10次独立试次建立初步记录；这不等于10次就足以证明稳健性。应保留所有失败及中断，不只选成功片段，并统一任务验收、速度、路径约束和反馈接口。

本次已经支持的结论是：**GPT-6 Astra能够在工具和人工监督下参与真实机器人操作，表现出任务理解、步骤组织、工具使用和交互纠错能力。精确空间控制、物理后果预测、实时接触反馈和可靠的自主完成仍需外部能力支撑与独立实验验证。** 这也是今天几轮实验对“大模型直接驱动具身”的主要发现。
